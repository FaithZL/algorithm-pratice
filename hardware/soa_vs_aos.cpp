...
const int ELEMS = 10'000'0000;

struct Person
{
    Person(const string& n, uint32_t a, uint32_t d)
            : name(n), age(a), dob(d) {}
...
{
    vector<string> names;
    vector<uint32_t> ages;
    vector<uint32_t> dobs;

    void addPerson(const string& n, uint32_t a, uint32_t d)
    {
        names.push_back(n);
        ages.push_back(a);
        dobs.push_back(d);
    ...
    {
        return accumulate(begin(ages), end(ages), (uint64_t)0,
                          [](auto sum, auto& p) { return sum + p; }) / ages.size();
    }

    uint64_t averageDob() const
    {
        return accumulate(begin(dobs), end(dobs), (uint64_t)0,
                          [](auto sum, auto& p) { return sum + p; }) / dobs.size();
    }
};
...
int main(int argc, char** argv)
{
    ...
    cout << "AoS duration " << duration_cast<microseconds>
